...
const App = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await Axios.get(`https://dog.ceo/api/${location.pathname.split('/').filter(elem => !elem.includes('=') && elem.length > 0).length > 1 ? 'breed/' + location.pathname.split('/').slice(2).filter(elem => !elem.includes('=')).join('/') + '/images' : 'breeds/image'}/random/${location.pathname.includes('img=') ? 12 : 13}`);
      console.log(`https://dog.ceo/api/${location.pathname.split('/').filter(elem => !elem.includes('=') && elem.length > 0).length > 1 ? 'breed/' + location.pathname.split('/').slice(2).filter(elem => !elem.includes('=')).join('/') + '/images' : 'breeds/image'}/random/${location.pathname.includes('img=') ? 12 : 13}`);

      setImages(location.pathname.includes('/img=') ? [`https://images.dog.ceo/breeds/${location.pathname.split('/')[2]}${location.pathname.split('/').slice(2).filter(elem => !elem.includes('=') && elem.length > 0).length > 1 ? '-' + location.pathname.split('/')[3] : ''}/${location.pathname.split('=')[1]}.jpg`, ...response.data.message] : response.data.message);
      console.log(location.pathname.includes('/img=') ? [`https://images.dog.ceo/breeds/${location.pathname.split('/')[2]}${location.pathname.split('/').slice(2).filter(elem => !elem.includes('=') && elem.length > 0).length > 1 ? '-' + location.pathname.split('/')[3] : ''}/${location.pathname.split('=')[1]}.jpg`, ...response.data.message] : response.data.message);
    }
          ...
          <div className='col'>
            ...
            <div className='row align-middle align-items-center'>
              <figure className="col-md">
                <Link to={`/dog-api/${images[0] ? images[0].split('/')[4].split('-').join('/') + '/img=' + images[0].split('/')[5].split('.', 1)[0] : ''}?`} className='text-center' data-size="1600x1067">
                  <img
                    alt="dog"
                    src={images[0]}
                  ...
                  <div className={`carousel-item${index === 0 ? ' active' : ''}`} key={`carousel-${index + 1}`}>
                    {images.slice(index + 1, index + 5).map((image, j) => (
                      <div className="col-md-3 mb-3" key={`carousel-item-${index + j + 1}`}>
                        <Link className="card" to={`/dog-api/${images[index + j + 1] ? images[index + j + 1].split('/')[4].split('-').join('/') + '/img=' + images[index + j + 1].split('/')[5].split('.', 1)[0] : ''}?`} data-size="1600x1067">
                          <img
                            alt="dog"
